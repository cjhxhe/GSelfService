...
public class ExcelUtils implements ExcelUtilsInterface {
    ...
    public ExcelUtils(InputStream is, String version) throws IOException {
    ...
    public static boolean isExcel(String pathname) {
        ...
        return EXT_XLS.equalsIgnoreCase(extension) || EXT_XLSX.equalsIgnoreCase(extension);
    }

    public static String getExcelVersion(String pathname) {
        if (pathname == null) {
            return DEF_VERSION;
        }
        String extension = FileUtils.getExtension(pathname);
        return EXT_XLS.equalsIgnoreCase(extension) ? LEGACY_VERSION : DEF_VERSION;
    }
